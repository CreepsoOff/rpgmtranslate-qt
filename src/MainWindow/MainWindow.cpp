    ...
    const auto postArchive = [this,
        ...
        if (!QFile::exists(
            ...
            if (!copied) {
                ...
                if (QFile::exists(
                        tempProjectSettings->resolvedSourcePath() +
                        u"/System.json"
                    )) {
...
auto MainWindow::syncSourceBaselineFromGameData(const bool appendForceRead)
    ...
    const BaseFlags readFlags =
        BaseFlags(projectSettings->flags & ~BaseFlags::SkipObsolete);
    ...
    markMissing("maps.txt"_L1, FileFlags::Map);
    markMissing("actors.txt"_L1, FileFlags::Actors);
    markMissing("armors.txt"_L1, FileFlags::Armors);
    markMissing("classes.txt"_L1, FileFlags::Classes);
    markMissing("commonevents.txt"_L1, FileFlags::CommonEvents);
    markMissing("enemies.txt"_L1, FileFlags::Enemies);
    markMissing("items.txt"_L1, FileFlags::Items);
    markMissing("skills.txt"_L1, FileFlags::Skills);
    markMissing("states.txt"_L1, FileFlags::States);
    markMissing("troops.txt"_L1, FileFlags::Troops);
    markMissing("weapons.txt"_L1, FileFlags::Weapons);
    markMissing("system.txt"_L1, FileFlags::System);

    if (projectSettings->engineType == EngineType::New) {
        markMissing("plugins.txt"_L1, FileFlags::Scripts);
        if (!QFile::exists(
                QFileInfo(sourcePath).dir().absolutePath() + u"/js/plugins.js"
            )) {
            skipMissingSelection.flags |= FileFlags::Scripts;
        }
    } else {
        markMissing("scripts.txt"_L1, FileFlags::Scripts);
    }
